    ...
    for (int i1 = 0; i1 < in_arrs.size(); i1++) {
      ...
      for (size_t output_i = 0; output_i < out_arrs[0].size(); output_i++) {
        ...
        if (backwards_attrs.attrs.op->name.compare("_backward_LRN") == 0) {
          ...
          // backwards inputs can be any value unless artifact is generated from forward pass
          for (size_t i = 0; i < backwards_attrs.num_inputs; i++)
            backwards_input[i] = outputs[0];
        }
